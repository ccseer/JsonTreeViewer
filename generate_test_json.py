...
def generate_json_file(filename, target_size_bytes):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        while current_size + template_len < target_size_bytes - margin:
            ...
            if count % 10000 == 0:
                ...
                sys.stdout.write(f"\rProgress: {percent:.2f}% ({current_size // (1024*1024)} MB)")
                sys.stdout.flush()
    ...
    print(f"\rDone: {filename} - Actual: {actual_size} bytes (Target: {target_size_bytes})")
...
if __name__ == "__main__":
    ...
    configs = [
        ("test_1k.json", 1024),
        ("test_11M.json", 11 * 1024 * 1024),
        ("test_101M.json", 101 * 1024 * 1024),
        ("test_1.01G.json", int(1.01 * 1024 * 1024 * 1024))
    ]
