...
### v1.1 (Current)
- ✅ CMake build system
- ✅ Seer-sdk 3.0 integration
- ✅ simdjson 4.6.3 upgrade
- ✅ Lazy loading with fetchMore
...
- [ ] Use `ondemand::value::location()` to capture byte position during node creation.
- [ ] Store offset alongside pointer for each node.
- [ ] When expanding a node, seek directly to its offset instead of re-parsing from root.
...
- [ ] Make the path label clickable to copy to clipboard.
...
- [ ] Consider showing byte offset alongside path for debugging (e.g., `/store/book/0 @ 1234 bytes`).
...
- [ ] Add search type selector (QComboBox or button group).
  ...
  - [ ] Update filter logic based on selection.
...
- [ ] Warn or no-op when `Expand All` would be too expensive.
...
- [ ] Avoid eagerly materializing huge objects with many fields.
...
- [ ] If practical, surface approximate error position such as line/column.
...
- [ ] Color value cells by JSON type.
- [ ] Keep the palette readable in both light and dark themes.
- [ ] Avoid using color as the only type indicator.

### 12. Object and array icons
...
- [ ] Detect likely Unix timestamps in numeric values.
- [ ] Show a readable datetime in tooltip or secondary text.
- [ ] Keep raw value unchanged.

### 14. URL helper

- [ ] Detect `http` and `https` string values.
- [ ] Add a context menu action such as `Open URL`.
- [ ] Do not auto-open on single click.
